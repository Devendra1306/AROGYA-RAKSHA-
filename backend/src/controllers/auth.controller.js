  ...
  setupProfile: async (req, res) => {
    ...
    try {
      ...
      const profileData = {
        ...
        waterIntake: Number(waterIntake) || 2.5,
        sleepDuration: Number(sleepDuration) || 7,
        stressLevel: stressLevel || 'Moderate',
        medicalConditions: Array.isArray(medicalConditions) ? medicalConditions : [medicalConditions || 'None'],
        allergies: Array.isArray(allergies) ? allergies : (allergies ? [allergies] : []),
        medications: Array.isArray(medications) ? medications : (medications ? [medications] : []),
        familyHistory: Array.isArray(familyHistory) ? familyHistory : (familyHistory ? [familyHistory] : []),
        emergencyContactName: emergencyContactName || '',
        emergencyContactPhone: emergencyContactPhone || '',
        emergencyContactRelationship: emergencyContactRelationship || 'Family',
        dietPreference: dietPreference || 'Vegetarian',
        foodRestrictions: Array.isArray(foodRestrictions) ? foodRestrictions : [],
        favoriteFoods: Array.isArray(favoriteFoods) ? favoriteFoods : [],
        dislikedFoods: Array.isArray(dislikedFoods) ? dislikedFoods : [],
        budgetPreference: budgetPreference || 'Medium Budget',
        healthGoal: healthGoal || 'Healthy Lifestyle',
        targetDuration: targetDuration || '3 Months',
        healthScore
      };
