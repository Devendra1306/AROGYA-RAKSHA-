...
const StructuredResponseCard = ({ text }) => {
  ...
  return (
    <div className="space-y-4 w-full text-slate-800 dark:text-slate-100 max-w-full">
      {/* 1. Condition Card */}
      {sections.condition && (
        <div className="bg-white dark:bg-slate-900 border border-outline-variant/30 dark:border-slate-800 rounded-2xl p-4 shadow-sm">
          <div className="text-[10px] uppercase tracking-wider text-outline font-bold mb-1">Possible Condition</div>
          <p className="text-label-md font-semibold leading-relaxed">{sections.condition}</p>
        </div>
      )}

      {/* Grid for Severity and Medicines */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* 2. Severity Card */}
        {sections.severity && (
          <div className={`border rounded-2xl p-4 shadow-sm flex flex-col justify-between ${severityColor} ${severityPulse}`}>
            <div>
              <div className="text-[10px] uppercase tracking-wider opacity-85 font-bold mb-1">Severity Level</div>
              <div className="text-lg font-extrabold flex items-center gap-1.5 mt-1">
                {sections.severity}
              </div>
            </div>
        ...
        {medicinesList.length > 0 ? (
          <div className="bg-white dark:bg-slate-900 border border-outline-variant/30 dark:border-slate-800 rounded-2xl p-4 shadow-sm">
            <div className="text-[10px] uppercase tracking-wider text-outline font-bold mb-2">Suggested Medicines</div>
              ...
              {medicinesList.map((med, idx) => (
                <div key={idx} className="flex items-start gap-2.5">
                  ...
                  <div className="text-label-md">
                    ...
                    {med.purpose && (
                      <p className="text-xs text-outline leading-tight mt-0.5">Used for: {med.purpose}</p>
                    )}
                  </div>
                </div>
              ))}
          ...
          sections.medicines && (
            <div className="bg-white dark:bg-slate-900 border border-outline-variant/30 dark:border-slate-800 rounded-2xl p-4 shadow-sm flex items-center justify-center text-outline text-label-md">
      ...
      {tipsList.length > 0 && (
        <div className="bg-white dark:bg-slate-900 border border-outline-variant/30 dark:border-slate-800 rounded-2xl p-4 shadow-sm">
          <div className="text-[10px] uppercase tracking-wider text-outline font-bold mb-2.5">Quick Care Tips</div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {tipsList.map((tip, idx) => (
              <div key={idx} className="flex items-center gap-2 text-label-md">
                <span className="text-emerald-500 font-bold select-none">✓</span>
                <span className="text-on-surface dark:text-slate-200">{tip}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {sections.doctor && (
        <div className="bg-blue-50 dark:bg-blue-950/20 border-l-4 border-blue-500 rounded-r-2xl p-4 text-blue-900 dark:text-blue-300 text-label-md shadow-sm">
          <div className="font-bold mb-1 flex items-center gap-1.5 text-blue-800 dark:text-blue-400">
            <span>🩺</span> Doctor Visit Recommendation
          </div>
      ...
      {/* 6. Emergency Alert */}
      {sections.emergency && (
        <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900 rounded-2xl p-4 text-red-950 dark:text-red-300 text-label-md shadow-md">
          <div className="font-extrabold mb-1.5 flex items-center gap-1.5 text-red-700 dark:text-red-400">
...
export default function MedicalAssistant() {
  ...
  useEffect(() => {
    ...
    setMessages([
      {
        role: 'assistant',
        content: "Hello! I am your Arogya Raksha AI Medical Assistant. I can analyze your symptoms, search disease details, explain medications, and provide natural-care remedies based on your profile context.\n\nDescribe how you are feeling or ask a health-related question to get started.",
        timestamp: new Date()
      }
    ]);
  }, []);
  ...
  return (
    <div className="max-w-[1280px] mx-auto px-margin-desktop py-stack-md text-slate-800 dark:text-slate-100 flex flex-col lg:flex-row gap-gutter h-[calc(100vh-64px)] transition-colors">
      ...
      {/* Sidebar: Conversation History (Only for logged-in users) */}
      ...
      <div className="flex-grow flex flex-col border border-outline-variant/30 dark:border-slate-800 rounded-2xl bg-white/70 dark:bg-slate-800/70 overflow-hidden shadow-md h-[calc(100%-192px)] lg:h-full">
          ...
          {messages.map((msg, idx) => (
            ...
            >
              ...
              >
                ...
                {/* AI response disclaimers & severity warnings */}
        ...
        {/* Input box form */}
        <form onSubmit={handleSend} className="p-4 bg-slate-50 dark:bg-slate-900 border-t border-outline-variant/30 flex gap-2">
          <input 
            type="text"
            value={inputText}
